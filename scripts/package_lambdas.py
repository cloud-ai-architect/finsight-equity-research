#!/usr/bin/env python
"""Build and deploy the FinSight Lambda package.
...
REGION = "ap-south-1"
PREFIX = "finsight-equity-research-dev-"
...
def build(staging: pathlib.Path) -> bytes:
    ...
    buf = pathlib.Path(tempfile.gettempdir()) / "finsight-lambda.zip"
...
def main() -> int:
    ...
    if args.dry_run:
        out = pathlib.Path(tempfile.gettempdir()) / "finsight-lambda.zip"
        print("dry-run: %s" % out)
        return 0
