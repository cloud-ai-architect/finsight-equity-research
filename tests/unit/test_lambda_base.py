"""Unit tests for FinSight Lambda plumbing and routing."""
...
class TestRunStage:
    def test_missing_parameter_returns_400(self):
        r = run_stage({}, ["ticker"], lambda d: "never")
        assert r["statusCode"] == 400
        assert "ticker" in json.loads(r["body"])["message"]

    def test_lists_every_missing_parameter(self):
        r = run_stage({}, ["ticker", "excerpt"], lambda d: "never")
        msg = json.loads(r["body"])["message"]
        assert "ticker" in msg and "excerpt" in msg

    def test_empty_string_counts_as_missing(self):
        r = run_stage({"ticker": ""}, ["ticker"], lambda d: "never")
        assert r["statusCode"] == 400

    def test_success_wraps_result(self):
        r = run_stage({"ticker": "x"}, ["ticker"], lambda d: {"ok": True})
    ...
    def test_model_error_maps_to_502(self):
        ...
        r = run_stage({"ticker": "x"}, ["ticker"], boom)
...
class TestInvokeJson:
    ...
    def test_model_error_is_a_medassist_error(self):
        assert issubclass(ModelError, FinSightError)
