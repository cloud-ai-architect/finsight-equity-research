...
locals {
  ...
  lambdas = {
    ...
    filings      = "${local.name_prefix}-filings"
    quant        = "${local.name_prefix}-quant"
    risk         = "${local.name_prefix}-risk"
    memo         = "${local.name_prefix}-memo"
    feedback     = "${local.name_prefix}-feedback"
  }
...
module "apigateway" {
  ...
  name_prefix     = local.name_prefix
  api_description = "FinSight equity research agent API"

  # One route per agent; the orchestrator is reachable too, which it
  # previously was not.
  routes = {
    research     = module.lambdas.function_arns["orchestrator"]
    filings      = module.lambdas.function_arns["filings"]
    quant        = module.lambdas.function_arns["quant"]
    risk         = module.lambdas.function_arns["risk"]
    memo         = module.lambdas.function_arns["memo"]
...
# window.FINSIGHT_API_URL and fell back to "https://api.example.com",
...
resource "aws_s3_object" "ui_config" {
  ...
  content = <<-JS
    ...
    window.FINSIGHT_API_URL = "${module.apigateway.api_url}";
    window.FINSIGHT_ENV = "${var.environment}";
  JS
